...
public class TicketCreateAccountActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ticket_create_account);
        sp = getSharedPreferences("login", MODE_PRIVATE);
        up = getSharedPreferences("UP", MODE_PRIVATE);
                        ...
                        public void done(List<UserInfo> object, BmobException e) {
                            if(e==null){
                                if (object != null && !object.isEmpty()) { //云端已经注册该account，进行提示
                                    Toast.makeText(TicketCreateAccountActivity.this, "账号已注册", Toast.LENGTH_SHORT).show();
                                    return;
                                }
                                ...
                                userInfo.save(new SaveListener<String>() {
                                    ...
                                    public void done(String s, BmobException e) {
                                        if (e == null) {
                                            // 用户信息保存成功
                                            Toast.makeText(TicketCreateAccountActivity.this, "User information saved to Bmob", Toast.LENGTH_SHORT).show();
                                            BmobQuery<UserInfo> query = new BmobQuery<UserInfo>();
                                            //查询playerName叫“比目”的数据
                                            query.addWhereEqualTo("account", account);
                                            ...
                                            query.findObjects(new FindListener<UserInfo>() {
                                                ...
                                                public void done(List<UserInfo> object, BmobException e) {
                                                    if(e==null && object!=null && !object.isEmpty()){
                                                        ...
                                                        long result = manager.saveUserInfo2DB(TicketCreateAccountActivity.this, username,account, sure_password);
                                                        if (result == -1){
                                                            Toast.makeText(TicketCreateAccountActivity.this, "Recreate!!Please try another account", Toast.LENGTH_SHORT).show();
                                                            return;
                                                        }
                                                        ...
                                                        edit.commit();
                                                        edit1.commit();
                                                        Intent intent = new Intent(TicketCreateAccountActivity.this, TicketsActivity.class);
                                            ...
                                            // 处理保存失败的情况
                                            Toast.makeText(TicketCreateAccountActivity.this, "Failed to save user information to Bmob", Toast.LENGTH_SHORT).show();
                                        }
                                    }
                                });
                            }else{
