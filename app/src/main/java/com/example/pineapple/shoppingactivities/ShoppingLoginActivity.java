...
public class ShoppingLoginActivity extends AppCompatActivity {
    ...
    private void initView() {
        ...
        bt_account.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent intent = new Intent(ShoppingLoginActivity.this, ShoppingCreateAccountActivity.class);
            ...
            public void done(List<UserInfo> object, BmobException e) {
                if(e==null){
                    if (object == null || object.isEmpty()) {
                        Toast.makeText(ShoppingLoginActivity.this, "账号或密码错误", Toast.LENGTH_SHORT).show();
                        return;
                    }
                    UserInfo userInfo = object.get(0);
                    SqlManager manager = new SqlManager();
                    //与云端做用户数据同步，加入本地数据库被清了，还能同步到云端的数据
                    if (manager.checkUserInfoInDB(ShoppingLoginActivity.this, account)) {
                        manager.updateUserInfo(ShoppingLoginActivity.this, userInfo);
                    } else {
                        manager.insertUserInfo(ShoppingLoginActivity.this, userInfo);
                    }
...
//                    }
                    //不管是否需要勾选记住登录账号，缓存都应该存储登录信息，只是勾选状态是false
                    saveUserInfo(account, pw, userInfo.getObjectId(), isChecked);

                    Intent intent = new Intent(ShoppingLoginActivity.this, ShoppingActivity.class);
